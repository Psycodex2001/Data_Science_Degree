...
# -*- coding: utf-8 -*-
"""
Created on Sat Feb 22 17:06:36 2025
...
# Exemplo 2: Minimizar o Custo Total com Restrição de Tempo
...
orcamento = 3000  # Orçamento máximo disponível para a viagem (em dólares)
tempo_maximo = 24  # Tempo máximo de viagem (em horas)
...
# --- 5. FUNÇÃO OBJETIVO ---
# Definimos o que queremos minimizar: o custo total da viagem.

# Calcula o custo total somando os custos das opções escolhidas
modelo.custo_total = Objective(
    ...
    sense=minimize  # Queremos minimizar o custo
)
...
# Restrição 3: O tempo total de viagem não pode ultrapassar o tempo máximo
modelo.limite_tempo = Constraint(
    expr=sum(modelo.x[i] * opcoes_ida[i]['tempo'] for i in modelo.I) +
         sum(modelo.y[j] * opcoes_volta[j]['tempo'] for j in modelo.J) <= tempo_maximo
)
...
if resultados.solver.status == 'ok': # Se encontrou uma solução...
    print("Custo total de viagem:", modelo.custo_total()) # Mostra o custo total
    print("Tempo total de viagem:", sum(modelo.x[i]() * opcoes_ida[i]['tempo'] for i in modelo.I) +
